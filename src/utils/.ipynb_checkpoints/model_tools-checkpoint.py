...
import utils.ciivae_etc as etc

# Code taken from CI-iVAE repo, with small modifications

def fit_ciivae(model, x_train, u_train, x_val, u_val,
    ...
    for epoch in range(1, num_epoch+1):
        ...
        for x_batch, u_batch in tqdm.tqdm(dataloader['train'],
            ...
            post_mean, post_log_var = etc.compute_posterior(z_mean, z_log_var, lam_mean, lam_log_var)
            post_sample = etc.sampling(post_mean, post_log_var)
            encoded_sample = etc.sampling(z_mean, z_log_var)
            ...
            kl_post_prior = etc.kl_criterion(post_mean, post_log_var, lam_mean, lam_log_var)
            kl_encoded_prior = etc.kl_criterion(z_mean, z_log_var, lam_mean, lam_log_var)
...
def extract_feature(result_path, x):
    ...
    z_sample = etc.sampling(z_mean, z_log_var)
    return z_sample
